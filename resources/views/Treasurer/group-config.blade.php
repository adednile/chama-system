...
<div class="space-y-6">
    ...
    <nav class="flex items-center gap-2 text-xs text-slate-500 mb-2">
        ...
        <span class="text-slate-800 font-medium">Group Configuration</span>
    </nav>
    ...
    <div class="premium-card rounded-2xl p-6 flex items-center justify-between flex-wrap gap-4 relative overflow-hidden">
        ...
        <div class="bg-digital-blue-50 border border-digital-blue-200 text-digital-blue-700 text-xs font-semibold px-4 py-2 rounded-xl flex items-center gap-1.5 shadow-sm">
    ...
    <form action="{{ route('treasurer.chama.config.update') }}" method="POST" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="premium-card rounded-2xl p-6 space-y-6">
            <h4 class="text-sm font-bold font-title text-digital-blue-600 border-b border-slate-100 pb-2 flex items-center gap-1.5">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="contribution_target" id="contribution_target" value="{{ old('contribution_target', $chama->contribution_target) }}" 
                           class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 focus:outline-none focus:border-digital-blue-500 transition">
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="collection_cutoff" id="collection_cutoff" value="{{ old('collection_cutoff', $chama->collection_cutoff ? \Carbon\Carbon::parse($chama->collection_cutoff)->format('Y-m-d') : '') }}" 
                           class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 focus:outline-none focus:border-digital-blue-500 transition">
                </div>
        ...
        <div class="premium-card rounded-2xl p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" max="1" name="savings_weight" id="savings_weight" value="{{ old('savings_weight', $chama->savings_weight) }}" 
                                   class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-800 focus:outline-none focus:border-digital-blue-500 transition">
                        </div>
            ...
            <div class="pt-6 border-t border-slate-100 flex justify-end">
                <button type="submit" class="bg-digital-blue-600 hover:bg-digital-blue-700 text-white px-6 py-2.5 rounded-xl text-xs font-bold shadow-md transition-all active:scale-95">
